...
This nested crate proves that heavy and infrequent routes from `embedded-alerts/eal-dioxus-web` can be built and deployed independently of the main Dioxus web server. It does not change the existing server entrypoint, initialize Dioxus, open a listener, create a WebSocket, or construct the main application/database state.

## Entry points

| Binary | API Gateway route | Required body field | Intended workload |
|---|---|---|---|
| `heavy_firmware_report` | `POST /api/heavy/firmware-report` | `device_id` | Generate a device firmware report without Dioxus startup. |
| `heavy_alert_replay` | `POST /api/heavy/alert-replay` | `alert_id` | Replay a historical alert through an isolated worker. |

Each binary is one AWS Lambda deployment unit:

- `heavy_firmware_report`
- `heavy_alert_replay`

The proof handler accepts API Gateway v1 and v2 request envelopes, requires an exact route and method, rejects base64 bodies, caps the serialized event at 1 MiB, emits structured Lambda tracing fields, and returns `202 Accepted`. Before production traffic, replace the accepted-response seam with a call into a framework-independent domain/service crate; do not link the Dioxus UI or server startup graph into these binaries.
