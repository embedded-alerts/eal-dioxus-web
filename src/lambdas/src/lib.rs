...
mod tests {
    ...
    fn accepts_exact_api_gateway_v2_route() {
        let response = dispatch(
            &payload("POST", SPEC.path, json!({"device_id":"value-1"})),
            "request-1",
            SPEC,
        );
        ...
        assert!(response["body"]
            ...
            .contains("heavy_firmware_report"));
    }
    ...
    fn rejects_route_method_and_missing_field() {
        assert_eq!(
            status(&dispatch(
                &payload("POST", "/wrong", json!({"device_id":"value-1"})),
                "r",
                SPEC,
            )),
            404,
        ...
        assert_eq!(
            status(&dispatch(
                &payload("GET", SPEC.path, json!({"device_id":"value-1"})),
                "r",
                SPEC,
            )),
            405,
        ...
        assert_eq!(
            status(&dispatch(
                &payload("POST", SPEC.path, json!({"device_id":"  "})),
                "r",
                SPEC,
            )),
            422,
    ...
    fn rejects_base64_and_oversized_events() {
        let mut encoded = payload("POST", SPEC.path, json!({"device_id":"value-1"}));
