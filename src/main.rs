...
fn App() -> Element {
    rsx! {
        main { class: "shell",
            ...
            h1 { "Embedded Alerts" }
            p { "Embedding-native monitoring and alerting that continuously matches user intents against newly ingested documents, feeds, pages, and streams." }
            section { class: "grid",
                article { h2 { "Production studio" } p { "Responsive SSR shell with an Axum WebSocket transport." } }
                article { h2 { "Primary API" } code { "/v1/alerts" } }
...
async fn index() -> Html<String> {
    ...
    Html(format!(r#"<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Embedded Alerts</title><style>body{{font-family:system-ui;margin:0;background:#0c111b;color:#eef2ff}}.shell{{max-width:960px;margin:auto;padding:4rem 1.5rem}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem}}article{{background:#182234;padding:1.25rem;border-radius:14px}}code{{color:#b8f7d4}}</style></head><body>{body}<script>const el=document.getElementById('live');const ws=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/ws`);ws.onopen=()=>el.textContent='Connected';ws.onmessage=e=>el.textContent=e.data;ws.onclose=()=>el.textContent='Disconnected';</script></body></html>"#))
}

async fn health() -> impl IntoResponse { axum::Json(serde_json::json!({"status":"ok","ui":"dioxus-ssr"})) }
async fn ws(upgrade: WebSocketUpgrade) -> impl IntoResponse { upgrade.on_upgrade(handle_ws) }
async fn handle_ws(mut socket: WebSocket) {
    let _ = socket.send(Message::Text("Embedded Alerts realtime channel ready".into())).await;
